...
public class Line {
    ...
    /**
     * calculates and returns the value of the middle point of the line.
     ...
     * @return the intersection point if there is one otherwise null
     */
    public Point intersectionWith(Line other) {
        ...
        if (Math.abs(x1 - x2) < k || Math.abs(x3 - x4) < k) {
            if (Math.abs(x1 - x2) < k && Math.abs(x3 - x4) < k) {
                if (Math.abs(x1 - x3) < k) {
                    if (Math.abs(y1 - y3) < k || Math.abs(y2 - y4) < k) {
                        if (y2 + k > y1 && y4 < y3 + k) {
                            return start;
                        }
                        if (y2 < y1 + k && y4 + k > y3) {
            ...
            if (Math.abs(x1 - x2) < k) {
                ...
                double c2 = (-1) * other.start.getX() * slope2 + other.start.getY();
                if ((x3 + k >= x1 && x4 <= x1 + k) || (x3 <= x1 + k && x4 + k >= x1)) {
                    if (y3 + k >= y1 && y4 <= y2 + k) {
            ...
            if (Math.abs(x3 - x4) < k) {
                ...
                double c1 = (-1) * start.getX() * slope1 + start.getY();
                if ((x1 + k >= x3 && x2 <= x3 + k) || (x1 <= x3 + k && x2 + k >= x3)) {
                    if (y1 + k >= y4 && y2 <= y3 + k) {
        ...
        if (Math.abs(slope1 - slope2) < k) {
            if (start.equals(other.start) && ((x2 <= x1 + k && x4 + k >= x3) || (x2 + k >= x1 && x4 <= x3 + k))) {
                return start;
            }
            if (end.equals(other.end) && ((x1 <= x2 + k && x3 + k >= x4) || (x1 + k >= x2 && x3 <= x4 + k))) {
                return end;
            }
            if (start.equals(other.end) && ((x2 <= x1 + k && x3 + k >= x4) || (x2 + k >= x1 && x3 <= x4 + k))) {
                return start;
            }
            if (end.equals(other.start) && ((x1 <= x2 + k && x4 + k >= x3) || (x1 + k >= x2 && x4 <= x3 + k))) {
                return end;
            }
            return null;
        }
        ...
        if ((x1 <= xi + k && xi <= x2 + k) || (x2 <= xi + k && xi <= x1 + k)) {
            if ((y1 <= yi + k && yi <= y2 + k) || (y2 <= yi + k && yi <= y1 + k)) {
                if ((x3 <= xi + k && xi <= x4 + k) || (x4 <= xi + k && xi <= x3 + k)) {
                    if ((y3 <= yi + k && yi <= y4 + k) || (y4 <= yi + k && yi <= y3 + k)) {
                        return intersection;
                    }
                }
            }
