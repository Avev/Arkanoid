...
public class Ball implements Sprite {
     ...
     * <p>
     *     the function checks the ball is gonig to cross one of his borders, if it will, then it changes the
     *     velocity's direction by changing it's dx or dy to it's minus.
     */
    public void moveOneStep() {
        ...
        if (colInf == null) {
            ...
            v = colInf.collisionObject().hit(this, colInf.collisionPoint(), v);
        }
    }

    /**
     * takes the input speed and changes it in a way that the bigger the radius smaller the velocity caps at 50.
    ...
    /**
     * checks if too add or substract 1 from the point.
     * @param collision the collision point
     * @return point
     */
    public Point newFun(Point collision) {
        double signX = Math.signum(center.getX() - collision.getX());
        double signY = Math.signum(center.getY() - collision.getY());
        return new Point(collision.getX() + signX, collision.getY() + signY);
    }
